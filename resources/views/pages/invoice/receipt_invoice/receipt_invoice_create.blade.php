...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <form action="{{route('receipt_invoice.store')}}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="exampleFormControlInput1">Invoice</label>
                    <div class="input-group">
                            <select class="form-control" name="id_inv" value="{{old('id_inv')}}" required>
                                <option value="">....</option>
                                @foreach($inv as $p)
                                <option value="{{$p->id}}">{{ \Carbon\Carbon::parse($p->tgl_inv)->format('Y-m-d')}}/{{$p->no_inv}}/{{$p->code_cust}}</option>
                                @endforeach
                            </select>
                    </div> 
            </div>
            ...
            <div class="mb-3">
                <label for="exampleFormControlInput1">Courier</label>
                    <div class="input-group">
                            <select class="form-control" name="code_courier" value="{{old('code_courier')}}" required>
                                <option value="">....</option>
                                @foreach($courier as $p)
                                <option value="{{$p->code_courier}}">{{$p->code_courier}} / {{$p->nama_courier}}</option>
                                @endforeach
                            </select>
                    </div> 
            </div>
