...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Product Transfer</h6>
    </div>
    <div class="card-body">
        ...
        @else
        @endif
        <form action="{{route('product_transfer.store')}}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="exampleFormControlInput1">Number</label>
                <div class="input-group">
                    <input class="form-control" id="exampleFormControlInput1" name="code_trf" type="text" required>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <input class="form-control" id="exampleFormControlInput1" name="desc_trf" type="text" required>
            </div>
            <div class="validation"></div>
            @error('desc_trf')
            <div class="alert alert-danger">{{ $message }}</div>
    ...
    */
    $('#addrow').click(function(e) {
        ...
        var newRow = $('<tr class="row-akun">' +
            '<td><select class="form-control select2" name="kode_barang[]"></select></td>' +
        ...
        $.ajax({
            ...
            success: function(data) {
                var selectKodeBarang = newRow.find('.select2[name="kode_barang[]"]');
                // Isi opsi SKU dari data yang diterima
                data.forEach(function(item) {
                    selectKodeBarang.append('<option value="' + item.kode_barang + '">' + item
                        .kode_barang + '</option>');
                });
                ...
                selectKodeBarang.on('change', function() {
                    ...
                    data.forEach(function(item) {
                        if (item.kode_barang === selectKodeBarang) {
