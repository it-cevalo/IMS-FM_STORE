        ...
        <form action="{{route('payment.store')}}" method="POST">
            ...
            <div class="mb-3">
                <label for="exampleFormControlInput1">Tax Invoice</label>
                    <div class="input-group">
                            <select class="form-control" name="id_tax_inv" value="{{old('id_tax_inv')}}" required>
                                <option value="#">....</option>
                                @foreach($tax_inv as $p)
                                <option value="{{$p->id}}">{{ \Carbon\Carbon::parse($p->tgl_inv)->format('Y-m-d')}}/{{$p->no_inv}}</option>
                                @endforeach
                            </select>
                    </div> 
            </div>
            ...
            <div class="mb-3">
                <label for="exampleFormControlInput1">Payment Via</label>
                    <div class="input-group">
                            <select class="form-control" name="payment_via" value="{{old('payment_via')}}" required>
                                <option value="#">....</option>
            ...
            <div class="mb-3">
                <label for="exampleFormControlInput1">Bank Account</label>
                    <div class="input-group">
                            <select class="form-control" name="bank_account" value="{{old('bank_account')}}">
                                <option value="#">....</option>
