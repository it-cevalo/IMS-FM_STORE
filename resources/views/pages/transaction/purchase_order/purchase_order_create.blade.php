...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <form id="formPO">
            ...
            <div class="mb-3">
                <label>Reason</label>
                <textarea class="form-control" name="reason_po" placeholder="Input Reason" required></textarea>
            </div>
...
<script>
    $(document).ready(function() {
        // Inisialisasi Select2 standar
        $('.select2').select2({ width: '100%' });
    
        // Buat array PRODUCTS sekali (safe) dari server
        ...
        // Fungsi buat options (kecuali kode_barang di 'except' array)
        function getProductOptions(except = []) {
            let options = `<option value="">Pilih Barang</option>`;
            PRODUCTS.forEach(function(p) {
                if (!except.includes(p.kode_barang)) {
                    // escape values just in case (Blade already escaped)
                    options += `<option value="${p.kode_barang}" data-nama="${p.nama}" data-harga="${p.harga}">${p.kode_barang}</option>`;
                }
            });
            return options;
        }
        ...
        $('#addrow').click(function(e) {
            ...
            // ambil semua kode_barang yang sudah dipilih
            let selected = [];
            ...
            // buat options tanpa kode_barang yg sudah dipilih
            let options = getProductOptions(selected);
    
            // row html (pakai class select2, tapi jgn segan untuk reinit setelah append)
            let row = `
            <tr>
                <td>
            ...
            // re-init select2 hanya pada select yang baru
        ...
        // Saat ganti kode_barang: isi nama & harga lalu refresh pilihan lain
        ...
        // Refresh opsi pada semua select agar Kode Barang yang sudah dipilih tidak muncul lagi
        function refreshKodeBarangOptions() {
            // kumpulkan semua Kode Barang terpilih
            let selected = [];
            $('.kode_barang').each(function() {
                let val = $(this).val();
                if (val) selected.push(val);
            ...
            // update masing-masing select
            $('.kode_barang').each(function() {
                let $sel = $(this);
                let current = $sel.val();
                // buat options yang membolehkan current tetap tampil
                let allowed = selected.filter(s => s !== current);
                let options = getProductOptions(allowed);
                $sel.html(options);
    
                // restore current jika ada
                if (current) {
                    $sel.val(current);
                } else {
                    $sel.val('');
                }
                // trigger change agar Select2 menampilkan value terbaru
        ...
        // Submit via AJAX (jika kamu masih pakai AJAX dari versi sebelumnya)
